...
//                .csrf(AbstractHttpConfigurer::disable);
                .csrf((csrf)->csrf.ignoringRequestMatchers("/saveMsg")); //.ignoringRequestMatchers(PathRequest.toH2Console())); //no need of it. Since we are using MySQL
//        http.headers((headers)->headers.frameOptions(frameOptionsConfig -> frameOptionsConfig.disable())); //similarly we dont need this frame since we are not using H2 database
        return http.build();
    }
